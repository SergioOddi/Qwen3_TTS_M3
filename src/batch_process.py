#!/usr/bin/env python3
"""
Script per elaborazione batch di file di testo.
Processa tutti i file .txt nella directory INPUT/ e genera audio in OUTPUT/.
"""
...
from generate_audio import generate_audio, load_config
...
def batch_process(input_dir, output_dir, config_path, skip_existing=True):
    """
    Processa tutti i file di testo in batch.
...
def main():
    parser = argparse.ArgumentParser(
        description='Elaborazione batch di file di testo per generazione audio'
    )
    parser.add_argument(
        '--input', '-i',
        default='INPUT',
